...
const network = require('../networks/okex-testnet-self.json');
...
module.exports = async function (deployer) {
    ...
    for (let token in network['Tokens']) {
        ...
        if (interestRateModeMap[interestRateModeKey] == null) { //如果这个参数的模型不存在，则新生成这个模型
            ...
            console.log('JumpRateModel address', res.address)
        }
        let cToken = null;
        await deployer.deploy(CErc20Delegate).then(function (res) { //首先发Deletate
            console.log('CErc20Delegate address', CErc20Delegate.address)
        });
        ...
        await deployer.deploy(CErc20Delegator,
            ...
            '0x0', { from: network.Admins.comptrollerAdmin }
