...
contract SpiralXLens {
    ...
    function getCompBalanceMetadata(SpiralX spx, address account)
        external
        view
        returns (CompBalanceMetadata memory)
    {
        return
            CompBalanceMetadata({
                balance: spx.balanceOf(account),
                votes: uint256(spx.getCurrentVotes(account)),
                delegate: spx.delegates(account)
            });
    }
    ...
    function getCompBalanceMetadataExt(
        ...
        uint256 balance = spx.balanceOf(account);
        comptroller.claimComp(account);
        uint256 newBalance = spx.balanceOf(account);
        uint256 accrued = comptroller.compAccrued(account);
        uint256 total = add(accrued, newBalance, "sum SPX total");
    ...
    function getCompVotes(
        ...
        for (uint256 i = 0; i < blockNumbers.length; i++) {
            res[i] = CompVotes({
                ...
                votes: uint256(spx.getPriorVotes(account, blockNumbers[i]))
            });
        }
        return res;
    }
